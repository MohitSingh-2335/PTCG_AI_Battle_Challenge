...
base_dir = os.path.dirname(os.path.abspath(__file__)) if '__file__' in globals() else os.path.join('/kaggle_simulations/agent', 'cg')
...
if os_name == 'Windows':
    lib_path = os.path.join(base_dir, "cg.dll")
elif os_name == "Darwin":
    lib_path = os.path.join(base_dir, "libcg.dylib")
elif platform.machine() in ('arm64', 'aarch64'):
    lib_path = os.path.join(base_dir, "libcg-arm64.so")
else:
    lib_path = os.path.join(base_dir, "libcg.so")
